...
import { FiDatabase, FiEye, FiBarChart2, FiClock } from "react-icons/fi";
...
// Helper Component: Status Widget
const StatusWidget = memo(({ icon, title, children }) => (
...
// Helper Component: Detection Card
...
const EmptyState = memo(() => {
  ...
  useEffect(() => {
    const animation = gsap.to(emptyRef.current, {
      opacity: 0.6,
      repeat: -1,
      yoyo: true,
      duration: 1.5,
...
export default function Fish() {
  ...
  // Effect to update the history when new detections arrive
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div data-animate className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
        ...
        <div className="grid lg:grid-cols-5 gap-8">
          ...
          >
            <h2 className="text-xl font-bold text-cyan-200 mb-4">Detection Log</h2>
            <div className="space-y-3 pr-2 overflow-y-auto max-h-[420px]">
              {detections.length === 0 ? (
                <EmptyState />
              ) : (
                detections.map((det, idx) => <DetectionCard key={idx} detection={det} />)
              )}
            </div>
          </div>
        </div>
